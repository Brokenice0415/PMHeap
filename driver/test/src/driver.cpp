...
int heap_mgr_allocate(HeapManager* hmgr, ShadowMemory* buffer, size_t size) {
  ...
  if (do_action()) {
    ...
    uintptr_t **ct = (uintptr_t**)malloc(sizeof(uintptr_t**));
    nvalloc_malloc_to(size, (void**)ct);
    ptr = (void*)*ct;
    free(ct);
...
retry:
  ...
  BEGIN_STMT;
  STMT("p[%d] = (void*)*tmptr", hmgr->smem.front);
  END_STMT;
...
int main(int argc, char** argv) {
  ...
  nvalloc_init(get_timestamp().c_str());
